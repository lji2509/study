...
# sequence
1. 유일한 데이터의 설정의 필요성
	데이터는 하나의 테이블에서 특정 컬럼의 경우에 식별할 수 있는 데이터를 입력이 필요한 경우가 많다.
	ex) 주민번호, 사번, 학번 등
2. key를 이용하여 만드는 여러가지 데이터들
	...
	10000 : 입사 순서별로 numbering
		...
		10002
		10003
	현재 어디까지 numbering이 되고 있는지만 알면 저장한 사원번호를 알지 못하더라도 효과적인 key인 사원번호를 중복되지 않게 입력이 가능하다.
	
	각 부서별로 연도별로 sequence(numbering할 수 있는)가 만들어진다면 효과적으로 key데이터를 처리할 수 있다.
	
3. sequence의 활용
	위와 같이 테이블에 종속되지 않고 식별하는 key데이터를 만들어서 입력 또는 수정할 때, 주로 활용된다.
	증가하는 숫자
	증가 단위
	최대 번호
	최소 번호
...
CREATE SEQUENCE seq_04
	...
	cache 2; --속도의 향상을 위해서 사용되는데, 일반적으로 cycle이 설정되면 cache 2 이상 처리되어야 한다.

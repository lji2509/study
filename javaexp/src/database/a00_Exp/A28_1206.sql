...
1. 계층형 sql이란 무엇인가?
*/
START WITH : 계층 구조 전개의 시작 위치를 지정하는 구문이다. 
	루트 데이터를 지정한다. (ACCESS)
CONNECT BY : 다음에 전개 될 자식 데이터를 지정하는 구문이다. 
	자식 데이터는 CONNECT BY 절에 주어진 조건을 만족해야 한다. (JOIN)
PRIOR : CONNECT BY 절에 사용되며, 현재 읽은 컬럼을 지정한다. 
	‘PRIOR 자식 = 부모’ 형태를 사용하면 계층 구조에서 자식 → 부모 방향으로 순방향 전개를 한다. 
	그리고 ‘PRIOR 부모 = 자식’ 형태를 사용하면 반대로 부모 → 자식 방향으로 전개하는 역방향 전개를 한다.
NOCYCLE : 데이터를 전개하면서 이미 나타났던 동일한 데이터가 전개 중에 다시 나타난다면 이것을 사이클 (Cycle) 이 형성되었다 라고 말한다. 
	사이클이 발생한 데이터는 런타임 오류가 발생한다. 그렇지만 NOCYCLE 을 추가하면 사이클이 발생한 이후의 데이터는 전개하지 않는다.
...
*/
계층형 sql이란 한 테이블에 담겨 있는 여러 레코드들이 서로 상하 관계(부모, 자식) 관계를 이루며 존재할 때,  이 관계에 따라 레코드를 상/하위 한 구조로 가져올 때 사용되는 SQL을 의미한다. 
하나의 사원정보 row는 사원번호(empno)와 관리자(mgr)가 있는데 이 mgr은 사원번호를 다시 가지고 있다. 
여기서 emp 테이블의 empno와 mgr를 보면 제일 최상위 구조에 7839 KING이 위치하고 있고 다음 2레벨에는 7782/7698/7566 3레벨에는 7934/7844/7900/7902이 위치한 트리 모형으로 이루어져 있다. 
...
*/
CREATE TABLE board(
    bno number PRIMARY KEY, --글번호는 식별자로 처리
    rbno NUMBER,
    title varchar2(50),
    name varchar2(30),
    redate date,
...
*/
게시판의 글번호 같은 경우는 순차적으로 생성되며 절대 중복되지 않는 Unique속성을 가지고 있다. 
시퀀스가 없다면 새로운 글이 생성될때마다 쿼리를 매번 실행하여 글 번호의 최대값을 확인 후 그보다 1큰 값을 계산해 새로운 글 번호를 INSERT한다.
...
INSERT INTO product3 values('Q'||to_char(sysdate,'yymm')||squ_product.nextval, '딸기', 12000);

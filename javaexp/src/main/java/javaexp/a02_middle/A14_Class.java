...
public class A14_Class {
	...
	public static void main(String[] args) {
		...
		// p01, p02, p03에는 그 heap영역의 메모리 주소를 stack영역에 참조명으로 할당
		System.out.println(p01);
		System.out.println(p02);
		System.out.println(p03);
		//javaexp.a02_middle.Person03@7de26db8
		//7de26db8 : 힙영역의 주소로 16진수로 저장
		...
		Person03 p05 = new Person03("김혜량", 1);
...
2. 클래스를 선언하면 .class까지 만들어지고, 객체를 사용하려면 main()을 통해서 생성자를 호출하여 객체를 생성하여야한다.
...
5. 객체가 생성되면 새로운 heap영역에 이 객체가 사용하는 메모리가 할당되고,
	그 heap영역의 메모리의 주소값을 참조변수에 할당하여 stack영역에 저장된다.
	stack/heap영역
	참조변수 : 어떤위치(heap영역에 객체가 있는 주소값)를 참조하는 변수
		참조변수안에 객체의 주소값이 할당되어있음
6. 클래스명 바로 밑에 변수를 데이터유형과 변수명으로 선언하며, 각 객체가 사용할 수 있는 변수가 선언된 것이다.
...
9. 매개변수가 있는 생성자는 일반적으로 필드값을 초기화시킬 때 주로 사용한다.
	ex) 우리나라 사람의 경우 사람이 태어나면 이름을 정하고, 나이1살을 정하듯이 객체를 생성하여 객체가 가지고 있을 속성(필드)를 초기화하여 사용하는데 활용한다.
	1) 필드값을 초기화할 때, 매개변수와 필드명이 같은 경우에 구분하기 위해서
		필드명은 this.필드명 = 매개변수명; 형식으로 매개변수로 넘어온 데이터를 필드명에 할당한다.
10. 기능 메서드는 주로 객체의 프로세스 처리, 입력값처리, 리턴값 처리등의 기능을 수행한다.
	ex) 눈으로 사물을 보면 시각정보가 입력되어 기억을 할 수 있게한다.
		클래스도 외부에서 입력되는 데이터를 필드에 저장할 수 있다.
	ex) 입으로 여러 친구들과 이야기를 할 수 있다. 기억된 정보를 음성으로 메시지로 바꾸어 외부에 전달한다.
		객체도 메서드를 통해서 필드의 데이터를 프로세스를 처리하여 외부에 리턴하여 전달할 수 있다.
	ex) 상황에 적절한 해결방법을 마련하고, 생각과 기억을 조합하여 효과적으로 대응전략을 만들고, 최적의 방법을 마련할 수 있다.
		객체도 메서드를 통하여 조건문과 반복문을 활용하여 프로세스를 만들고, 그 결과를 저장하여 추후에 다른 기능메서드를 통해서 처리할 때 활용할 수 있다.
 */
class Person03 {
//	 Person03(){}  //기본생성자가 자동으로 실행
	...
	//System.out.println("이야기할 내용 : " + p.04.speakWithMouth());
	...
	Person03() { //main() 메서드에서 new Person03()이라고 호출하는 순간 해당 중괄호 블럭에 있는 내용 수행

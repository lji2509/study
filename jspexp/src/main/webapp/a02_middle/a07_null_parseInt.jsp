...
<%--
# 요청값의 null에 대한 처리
1. 일단 form객체를 통해 요청값을 처리하여 바로 화면에 나타내야 하는 경우 초기에는 요청값이 없기에 처리해야 한다.
	처리하지 않으면 nullpointerexception 발생
...
1. Integer.parseInt() : 숫자형 문자열에 대한 숫자형 변환 처리
	모든 요청값은 문자열이기에 숫자처리를 하려면 형변환이 필요로 한다.
	String num1S = request.getParameter("num1");
	int num1=0; //숫자데이터 초기값을 설정해주고,
	//요청값이 있을 때, 형변환 처리한다.
	if(num1S!=null {

...
<%
	...
	//3. session scope : 변수를 session범위로 설정하는데, 이 범위는 서버와 클라이언트가 접속된 범위 안에서 처리할 때 사용한다.
	session.setAttribute("sessVar","오길동(session)");
	//4. application scope : 변수를 application 범위로 설정하는데, 이 범위 서버가 유지가 유지될 때까지 사용할 수 있다.

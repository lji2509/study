...
<%--
# Cookie
1. 서버와 클라이언트 관계에 있어서 발생하는 데이터를 클라이언트(브라우저)에 저장하는 객체를 말한다.
2. 처리단계 및 생성
	1) client ==> server : 브라우저를 통해 서버에 접속
							이 두 관계에서 발생하는 정보
							point를 증가/특정한 사용자 접속된 정보 등
							물건을 구매전에 임시로 장바구니(카트)에 담은 정보
...
<body>
	...
	<%
	//쿠키는 일단, 서버에서 만들어진다.
	Cookie ck1 = new Cookie("name", "himan");
	//2. 만들어진 쿠키는 명렁어에 의해 client에 보내어진다.
	response.addCookie(ck1);
	%>
	<table>
		<tr><th>쿠키의 이름</th><td><%=ck1.getName() %></td></tr>
		<tr><th>쿠키의 값</th><td><%=ck1.getValue() %></td></tr>
		<tr></tr>
	</table>

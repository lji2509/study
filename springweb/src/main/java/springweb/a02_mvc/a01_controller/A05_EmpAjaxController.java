...
public class A05_EmpAjaxController {
	...
	//초기화면 호출 http://localhost:8000/springweb/empAjax.do
	...
	//ajax데이터 처리
	//초기화면 호출 http://localhost:8000/springweb/empAjax1.do
	@GetMapping("/empAjax1.do")
	public String empAjax(Emp sch, Model d) {
		d.addAttribute("empList", service.getEmpList(sch));
		return "pageJsonReport"; //json View 호출
	}
	
	//초기화면 호출 http://localhost:8000/springweb/empAjaxDetail.do?empno=7654
	...
	public String empAjaxDetail(int empno, Model d) {
		...
		return "pageJsonReport"; //json View 호출
		//ArrayList형 empList를 json형식으로 json view에 의해 변경 처리해준다.
	}

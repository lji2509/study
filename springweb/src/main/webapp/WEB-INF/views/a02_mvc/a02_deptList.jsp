...
<div class="container">
	<form id="frm01" class="form-inline"  method="post">
	  	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
		    <input class="form-control mr-sm-2" placeholder="부서명" name="dname" value="${dept.dname}" />
		    <input class="form-control mr-sm-2" placeholder="부서위치" name="loc" value="${dept.loc}" />
		    <button class="btn btn-info" type="submit">Search</button>
		    <button data-toggle="modal" onclick="regFun()" data-target="#exampleModalCenter" class="btn btn-success" type="button">등록</button>
	 	</nav>
	</form>
	   ...
	   <thead>
	    ...
	    <tbody>
	    	<c:forEach var="dept" items="${dlist}">
	   		 	<tr onclick="detail(${dept.deptno},'${dept.dname}','${dept.loc}')" data-toggle="modal" data-target="#exampleModalCenter">
	    			<td>${dept.deptno}</td>
	    			<td>${dept.dname}</td>
	    			<td>${dept.loc}</td>
	    		</tr>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-secondary" id="uptBtn">부서수정</button>
